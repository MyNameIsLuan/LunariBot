...
client.on("guildMemberRemove", member =>{
    if(member.guild.id === "463006280894906369"){
        const leave = new Discord.RichEmbed()
        .setAuthor(member.user.tag)
        .setDescription(":outbox_tray: Volte logo!")
        .setThumbnail(member.user.avatarURL)
        .setColor("#00a4ce")
        .setFooter("Hype BOT - copyright")
      client.channels.get("463072723611025409").send(leave)
...
client.on("guildMemberAdd", member =>{
    if(member.guild.id === "463006280894906369"){
        ...
        .setColor("#00a4ce")
        .setFooter("Hype BOT - copyright")
      client.channels.get("463072662130917396").send(join)
